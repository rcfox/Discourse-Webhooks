  ...
  SiteSetting.webhooks_registered_events.split('|').each do |event_name|
    ...
    DiscourseEvent.on(event_name.to_sym) do |*params|
      ...
      begin
        ...
        # topic_uri.query = ({:api_key => SiteSetting.webhooks_discourse_api_key, :api_username => SiteSetting.webhooks_discourse_api_username})
        topic_http = Net::HTTP.new(topic_uri.host, topic_uri.port)
        topic_http.add_field('Api-Key',SiteSetting.webhooks_discourse_api_key) 
        topic_http.add_field('Api-Username',SiteSetting.webhooks_discourse_api_username) 
